...
export TOOL_VERSION=5.6.4
export PROJECT_VERSION=1.12
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=

set -- "$@" assemble uploadArchives -DdisableTests -x checkSigningRequirements 
...
[[ ! -f settings.gradle ]] && echo "rootProject.name = 'json-patch'" > settings.gradle
...
if [ -n "" ]; then
  echo "Creating tag 1.12 to match enforced version"
  git tag -m 1.12 -a 1.12 || true
fi
